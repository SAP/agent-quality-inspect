...
def serialize_to_conversation(
    ...
    if evaluation_result is not None and hasattr(evaluation_result, "milestone_mapping"):
        for milestone_index, (
            snapshot_index,
    ...
    # Patch: handle minefield_mapping safely
    snapshot_indices_to_minefields: dict[int, list[dict[str, Any]]] = defaultdict(list)
    if evaluation_result is not None and hasattr(evaluation_result, "minefield_mapping"):
        for minefield_index, (
            snapshot_index,
