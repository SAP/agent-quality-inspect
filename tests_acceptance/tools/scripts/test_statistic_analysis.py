...
from agent_inspect.models.metrics import SubGoalValidationResult, SubGoal
...
def error_analysis_data_sample_1() -> ErrorAnalysisDataSample:
    ...
    dummy_subgoal1_validations = SubGoalValidationResult(
        is_completed=False,
        explanations=['DUMMY STRING', '\n\nGRADE: I', '\n\nTherefore, the task is INCOMPLETE.\n\nGRADE: I', '\n\nGRADE: I', '\n\nGRADE: I', ' the agent has validly terminated their portion of the dialogue per instructions.\n\nGRADE: C'],
        sub_goal=SubGoal(details='Agent books one way economy flight from DTW to SEA on 2024-05-17 with flights HAT097 and HAT251 for passenger Ivan Smith, no baggage, no insurance.')
    )
...
def error_analysis_data_sample_2() -> ErrorAnalysisDataSample:
    ...
    dummy_subgoal1_validations = SubGoalValidationResult(
        is_completed=False,
        explanations=['DUMMY STRING', '\n\nGRADE: C', '\n\nTherefore, the task is INCOMPLETE.\n\nGRADE: C', '\n\nGRADE: C', '\n\nGRADE: C', ' the agent has validly terminated their portion of the dialogue per instructions.\n\nGRADE: C'],
        sub_goal=SubGoal(details='Agent books one way economy flight from DTW to SEA on 2024-05-17 with flights HAT097 and HAT251 for passenger Ivan Smith, no baggage, no insurance.')
    )
    # Judge score for subgoal 2: [0,0,0,0,0]
    dummy_subgoal2_validations = SubGoalValidationResult(
